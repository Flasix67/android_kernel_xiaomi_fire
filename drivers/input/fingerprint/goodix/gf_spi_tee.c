...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	gf_debug(ERR_LOG, "%s, from dts pinctrl\n", __func__);
...
#endif
#if 0
	gf_dev->fpsensor_goodix_power_on = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fpsensor_goodix_power_on");
	...
	gf_dev->fpsensor_goodix_power_off = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fpsensor_goodix_power_off");
	if (IS_ERR(gf_dev->pins_reset_high)) {
		ret = PTR_ERR(gf_dev->pins_reset_high);
	...
	if (IS_ERR(gf_dev->pins_reset_low)) {
   	...
   	gf_dev->pins_spi_cs_high = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fpsensor_goodix_cs_high");
	if (IS_ERR(gf_dev->pins_spi_cs_high)) {
		ret = PTR_ERR(gf_dev->pins_spi_cs_high);
		gf_debug(ERR_LOG, "%s can't find fingerprint pins_spi_cs_high\n", __func__);
		return ret;
	}

   	gf_dev->pins_spi_cs_low = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fpsensor_goodix_cs_low");
	if (IS_ERR(gf_dev->pins_spi_cs_low)) {
		ret = PTR_ERR(gf_dev->pins_spi_cs_low);
		gf_debug(ERR_LOG, "%s can't find fingerprint pins_spi_cs_low\n", __func__);
		return ret;
	}
...
static void gf_irq_gpio_cfg(struct gf_device *gf_dev)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,goodix-fp");
	if (node) {
                gf_dev->irq_num = of_get_named_gpio(node, "int-gpios", 0);
                gf_dev->irq = gpio_to_irq(gf_dev->irq_num);
...
static int gf_fb_notifier_callback(struct notifier_block *self,	unsigned long event, void *data)
{
	struct gf_device *gf_dev = NULL;
...
static irqreturn_t gf_irq(int irq, void *handle)
{
	...
	__pm_wakeup_event(gf_dev->fp_wake, WAKELOCK_HOLD_TIME);
...
static int gf_probe(struct platform_device  *pdev)
{
	...
	buck = regulator_get(gf_dev->device, "fp_vldo28");
	if (buck ==NULL) {
		gf_debug(ERR_LOG, "regulator_get fail");
		goto err_buf;
	}
    ...
    //pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->fpsensor_goodix_power_on);
	gf_get_sensor_dts_info();

	/*enable the power*/
	pr_err("%s %d now get dts info done!",__func__, __LINE__);
	//gf_hw_power_enable(gf_dev, 1);
	pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_spi_cs_low);
	mdelay(1);
    ...
    gf_dev->fp_wake = wakeup_source_create("fp_wakesrc");
    wakeup_source_add(gf_dev->fp_wake);
...
static int gf_remove(struct platform_device *pdev)
{
    ...
    wakeup_source_remove(goodix_dev.fp_wake);
...
err_buf:
	...
	//pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->fpsensor_goodix_power_off);
